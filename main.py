...
### 库有但经分析不采纳的功能
- **科里奥利力**: 需 latitude/azimuth 输入，对 >800m 远程有影响，留待后续
- **多层风区**: 仅单层 Wind，多段风距场景低频
- **cant_angle / 归零大气分离**: 边缘场景，UI 复杂度不划算
- **库自带 matplotlib 绑图** (hit_result_as_plot): 自建 Figure 无法嵌入 TkAgg，远不如自绘
...
### 5 个分析 Tab
- **Tab 1 单条弹道**: CFD 彩虹渐变色轨迹图 + 速度副轴 + 16 列详细数据表（含 OGW/OGW 和 flag 标记），分桶行颜色标记（归零-橙/顶点-绿/跨音速-蓝），鼠标 hover 十字光标 + click 高亮吸附
- **Tab 2 弹道分析**: 多弹药叠加轨迹图 + 速度副轴 + 13 列对比表（含截面比动能、后坐冲量、PBR 0.3/1.0/1.5m、超音速距离），曲线交点自动检测（高度+速度双轴），柱头自适应距离前缀
- **Tab 3 动能分析**: 多弹药动能曲线 + 3 个 Mach 点标记 + 9 列分析表，曲线交点检测，截面比动能支持距离插值
- **Tab 4 风偏分析**: 多弹药风偏曲线 + 对比/详细双模式表格（复选框切换），对比表 6 列、详细表 5 列分桶带颜色
- **Tab 5 阻力分析**: 多弹药阻力(Mach)曲线 + dF/dM 导数副轴 + G7 极值点标记 + 6 列数据表（BC 自适应带阻力表类型前缀），曲线交点检测
...
- BC ↔ i (弹形系数) 双向联动编辑
- 截面比动能在 Tab 2/3 表格中支持距离插值（无距离→枪口值，有距离→插值）

### 图表交互
- 5 个 Tab 全部 blit 加速 hover，60ms 防抖重绘
- hover 十字光标 + 数值提示，click 高亮吸附特殊点（归零/顶点/跨音速/交点/G7极值）
- 曲线交点: 多曲线自动检测，混合色标记（不进入图例）
- 中文化 Matplotlib 工具栏（重置/后退/前进/平移/缩放/子图/保存/复制到剪贴板）
...
- **自定义阻力表**: 任意 Mach-CD 数据点输入，BC 自动禁用
- **仰角/俯角**: 非水平射击支持
- **缠距 + 旋向**: 左旋/右旋选择

### 表格通用功能
- **右键隐藏列**: 所有 7 个 Treeview 通用 ColumnToggle 类，原生 checkbutton 菜单
- **自适应列宽**: 根据表头+首行+末行自动调整，60~120px 范围

### 单位系统
- 3 种单位制: 公制 / 英制 / 混合制
- 16 个 PreferredUnits 槽位全覆盖
- UI 标签随单位制动态切换符号
"""
